	...
	if (typeof indexedDB == 'object') {
		...
		return {
			open: function (gameName_n) {
				...
				if (db == null) {
					var request = indexedDB.open("my db", 1);  // New parameter to open.
					request.onupgradeneeded = function(event) {
						// If the specified version is greater than the current version, an upgradeneeded event will fire first.
						db = event.target.result;
						var versionChangeTransaction = event.target.transaction;
						db.createObjectStore("my db");
						// create other object stores, indices, etc
						versionChangeTransaction.oncomplete = function (e) {
